// B2C Students configuration

export const studentsConfig = {
  hero: {
    badge: "12,000+ SA Students Improving Their Grades",
    headline: "Your AI Study Buddy for Matric Success",
    subheadline:
      "24/7 homework help and exam prep for R99/month. Start free!",
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "See Plans",
  },

  stats: [
    {
      value: "12,000+",
      label: "Students",
      description: "Using StudyBuddy daily",
    },
    {
      value: "45%",
      label: "Improvement",
      description: "Average grade increase",
    },
    {
      value: "R99",
      label: "Per Month",
      description: "Less than a coffee/week",
    },
    {
      value: "All",
      label: "Subjects",
      description: "Maths, Science & more",
    },
  ],

  features: [
    {
      icon: "MessageSquare",
      title: "AI Tutor 24/7",
      description:
        "Get instant homework help anytime. Never study alone again.",
      color: "accent",
      highlights: [
        "Instant explanations",
        "Step-by-step guidance",
        "Available 24/7",
        "All subjects covered",
      ],
    },
    {
      icon: "BookOpen",
      title: "All Your Subjects",
      description:
        "Maths, Science, Languages & more. Full CAPS & IEB curriculum coverage.",
      color: "secondary",
      highlights: [
        "Mathematics",
        "Physical Science",
        "Life Science",
        "English & Afrikaans",
        "Accounting",
        "History & Geography",
      ],
    },
    {
      icon: "TrendingUp",
      title: "Track Your Progress",
      description:
        "See your improvement over time and know where to focus.",
      color: "primary",
      highlights: [
        "Subject-by-subject tracking",
        "Strength identification",
        "Goal setting",
        "Achievement badges",
      ],
    },
  ],

  additionalFeatures: [
    {
      icon: "Clock",
      title: "Exam Prep",
      description: "Practice past papers and review key topics.",
    },
    {
      icon: "CheckCircle",
      title: "Homework Help",
      description: "Get explanations that help you understand.",
    },
    {
      icon: "Smartphone",
      title: "Mobile Friendly",
      description: "Study anywhere on phone, tablet, or computer.",
    },
    {
      icon: "Award",
      title: "Proven Results",
      description: "45% average improvement in 3 months.",
    },
  ],

  subjects: [
    "Mathematics",
    "Physical Science",
    "Life Science",
    "English",
    "Afrikaans",
    "Accounting",
    "History",
    "Geography",
    "Business Studies",
    "Economics",
  ],

  howItWorks: [
    {
      number: "01",
      title: "Sign Up Free",
      description:
        "Create your account in 30 seconds. Start with a free week to try everything. No credit card needed.",
      color: "accent",
    },
    {
      number: "02",
      title: "Choose Your Subjects",
      description:
        "Tell us what subjects you're studying. We'll customize your experience for your curriculum.",
      color: "secondary",
    },
    {
      number: "03",
      title: "Start Learning",
      description:
        "Ask questions, get homework help, prepare for exams. Your AI tutor is ready 24/7 whenever you need help.",
      color: "primary",
    },
    {
      number: "04",
      title: "See Your Progress",
      description:
        "Watch your grades improve week by week. Track your progress and celebrate your achievements.",
      color: "accent",
    },
  ],

  testimonials: [
    {
      quote:
        "My maths improved from 52% to 78% in one term! StudyBuddy explains things in a way that actually makes sense to me.",
      author: "Lerato M.",
      role: "Grade 12 Student",
      school: "Pretoria, Gauteng",
      rating: 5,
    },
    {
      quote:
        "I use it every night for homework. It's like having a personal tutor who never gets tired of my questions. Worth every rand!",
      author: "Josh K.",
      role: "Grade 11 Student",
      school: "Cape Town, Western Cape",
      rating: 5,
    },
    {
      quote:
        "I was failing Physical Science. Now I'm getting 70s and actually understand what's happening. Game changer for matric!",
      author: "Zanele N.",
      role: "Grade 12 Student",
      school: "Durban, KwaZulu-Natal",
      rating: 5,
    },
  ],

  parentTestimonials: [
    {
      quote:
        "My daughter's confidence has skyrocketed. She actually enjoys studying now. R99/month is a steal compared to tutors.",
      author: "Mrs. Patel",
      role: "Parent",
      school: "Johannesburg",
    },
    {
      quote:
        "As a parent, I love the progress reports. I can see exactly where my son is improving and where he needs more help.",
      author: "Mr. Botha",
      role: "Parent",
      school: "Cape Town",
    },
  ],

  cta: {
    headline: "Ready to Improve Your Grades?",
    subheadline:
      "Join 12,000+ students improving their grades. Start free!",
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "See Pricing",
    benefits: [
      "Free for 7 days",
      "No payment required",
      "Cancel anytime",
    ],
  },

  faq: [
    {
      question: "Is it really free for a week?",
      answer:
        "Yes! Start a free 7-day trial with full access. No payment required. If you love it, subscribe for R99/month.",
    },
    {
      question: "Will this help me with my matric exams?",
      answer:
        "Absolutely! We help with past papers, explain concepts, and provide exam prep. Students improve by 30-40% on average.",
    },
    {
      question: "What subjects do you cover?",
      answer:
        "All subjects - Maths, Sciences, Languages, Humanities, and more. One subscription covers everything you're studying.",
    },
  ],
};
